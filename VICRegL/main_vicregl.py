...
def main(args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, inputs in enumerate(train_loader, start=epoch * len(train_loader)):
            ...
            if (
                ...
                for k, v in stats.items():
                    run[npt_logger.base_namespace][f"train/{k}"].append(v)
        ...
        if (epoch + 1) % args.eval_freq == 0:
            if args.evaluate:
                evaluate(model, logs, val_loader, args, epoch, lr, stats_file, gpu, run, npt_logger)


def evaluate(model, logs, val_loader, args, epoch, lr, stats_file, gpu, run, npt_logger):
    model.eval()
    ...
    if args.rank == 0:
        ...
        for k, v in stats.items():
            run[npt_logger.base_namespace][f"val/{k}"].append(v)
