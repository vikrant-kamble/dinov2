...
class BenchmarkModule(pl.LightningModule):
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(self.dataloader_kNN, total=len(self.dataloader_kNN)):
                ...
                if self.gpus > 0:
                    img = img.cuda()
                    target = target.cuda()
                feature = self.get_features(img).squeeze()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if hasattr(self, 'feature_bank') and hasattr(self, 'targets_bank'):
            ...
            feature = self.get_features(images).squeeze()
            feature = F.normalize(feature, dim=1)            
            pred_labels = knn_predict(feature, self.feature_bank, self.targets_bank, len(self.classes), self.knn_k, self.knn_t)
            num = images.size(0)
